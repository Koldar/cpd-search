#ifndef _CPDSEARCH_CPDSEARCHVIDEOPRODUCERLISTENER_HEADER__
#define _CPDSEARCH_CPDSEARCHVIDEOPRODUCERLISTENER_HEADER__
...
namespace pathfinding::search::listeners {
    ...
    class CpdSearchVideoProducerListener: public CpdSearchListener<G, V, STATE>, public StateVideoProducer<G, V, EORIGINAL, EPERTURBATED, MAPTYPE, IMAGETYPE> {
    public:
        using This = CpdSearchVideoProducerListener<G, V, EORIGINAL, EPERTURBATED, STATE, MAPTYPE, IMAGETYPE>;
        using Super1 = CpdSearchListener<G, V, STATE>;
        using Super2 = StateVideoProducer<G, V, EORIGINAL, EPERTURBATED, MAPTYPE, IMAGETYPE>;
    public:
        CpdSearchVideoProducerListener(const IImmutableGraph<G,V,EORIGINAL>& originalGraph, const IImmutableGraph<G, V, EPERTURBATED>& perturbatedGraph, const MAPTYPE& map, const function_t<EPERTURBATED,cost_t>& costFunction): Super2{originalGraph, perturbatedGraph, map, costFunction} {
        }
        virtual ~CpdSearchVideoProducerListener() {

        }
        CpdSearchVideoProducerListener(const This& o) = default;
        CpdSearchVideoProducerListener(This&& o) = default;
    ...
    public:
        ...
        virtual void onNodeExpanded(int iteration, const STATE& node) {
            ...
            boost::filesystem::path p{scout("./cpdsearch_videoframe_iteration_", iteration, ".jpeg")};
            this->addImage(p);
        }
