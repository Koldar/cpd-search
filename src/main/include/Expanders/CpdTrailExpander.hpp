...
namespace pathfinding::search {
    ...
    protected:
        ...
        fractional_cost minDiscount;
        fractional_cost maxDiscount;
        /**
         ...
         */
        fractional_cost discountDecay;
    public:
        CpdTrailExpander(const cpp_utils::graphs::IImmutableGraph<G, V, PerturbatedCost>& graph, CpdFocalHeuristic<STATE, G, V>& cpdHeuristic, fractional_cost minDiscount, fractional_cost maxDiscount, fractional_cost discountDecay): graph{graph}, cpdHeuristic{cpdHeuristic}, minDiscount{minDiscount}, maxDiscount{maxDiscount}, discountDecay{discountDecay} {
    ...
    public:
        virtual cpp_utils::vectorplus<std::pair<STATE&, cost_t>> getSuccessors(const STATE& state, Supplier& supplier) {
            ...
            // represents the distance to the endpoint of the earliest perturbation!
            // the state generated by following the cpd has their discount resetted
            for (int i=result.lastIndex(); i>=0; --i) {
                result[i].first.setDiscount(this->getDiscount(
                    result[i].second, 
                    costFromStateToCurrentTrail,
                    this->minDiscount,
                    this->maxDiscount
            ...
            for (auto outEdge : this->graph.getOutEdges(state.getPosition())) {
                ...
                auto successor = supplier.getState(outEdge.getSinkId());
                successor.setDiscount(this->getDiscountFromDiscountedState(successor));
                result.add(std::pair<STATE&, cost_t>{
                    successor,
    ...
    private: 
        fractional_cost getDiscountFromDiscountedState(const State& s) const {
            getLeftGaussian

            if (s.getDiscount() == 1) {
                return 1;
            }
            auto m = 1 - this->discountDecay.getRatio();
            return fractional_cost{m * s.getDiscount().getRatio() + this->discountDecay.getRatio(), 1e-3};
        }
        fractional_cost getDiscount(cost_t distance, cost_t maxDistance, fractional_cost minDiscount, fractional_cost maxDiscount) const {
            //the discount is computed via a linear equation
            auto m = (minDiscount.getRatio() - maxDiscount.getRatio())/(static_cast<double>(maxDistance));
            result.setDiscount(fractional_cost{-m*distance + maxDiscount, 1e-3});
            return result;
        }
    public:
